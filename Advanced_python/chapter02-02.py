...
# 매직 메소드란
# 1) 클래스 안에서 정의할 수 있는 스페셜 메소드이다. 빌트인 타입(built in type)과 같은 기능을 작동한다.
# 2) +, -, >, < 등의 오퍼레이터에 대해서 각각의 데이터 타입에 맞는 메소드로 오버로딩하여 백그라운드에서 연산을 한다.
# 3) 메소드 이름 앞뒤로 "__" 더블 언더 스코어를 붙인다.
