...
# 반복형 확인
# dir로 namespace 안에 iter라는 값이 존재하는지 확인 가능
# hasattr, isinstance로 __iter__ 속성 확인 가능

print('EX1-3 -', hasattr(t,'__iter__')) # __iter__라는 속성을 가지고 있는지 확인
...
class WordSplitGenorator:
    ...
    def __repr__(self):
        return 'WordsplitGenorator(%s)' % (self._text)

wg = WordSplitGenorator('Who says the nights are for sleeping')
wt = iter(wg)
...
def generator_ex1():
    ...
    yield 'BBB'
    print('end')
# next함수에서 yield를 만나면 멈춘다. 그리고 다시 next를 호출하면 다음 yield를 만나면 멈춘다.

temp = iter(generator_ex1())

# print('EX4-1 -',next(temp)) # next : 하나씩 호출
...
for v in generator_ex1():
    pass
    # print('EX4-3 -', v)
# for 문을 실행하면 StopLiteration이 실행되기 전까지만 반복수행한다.
# 안정하게 사용가능
# for, while을 통해서 Generatior를 주로 사용
# 모든 자료구조를 저장하는게 아니라 yield 까지만 실행하고 보여주는 식이기 때문에 메모리를 절약할 수 있다.
...
print('EX5-1 -',temp2)
print('EX5-2 -',temp3) # generator 반환 # next method로 호출하기 전까지는 만들지 않음
...
print('EX6-10 -', list(gen7)) # 튜플 형태로 쪼개줌(개별적으로)

# 연산(경우의 수)
gen8 = itertools.product('ABCDE',repeat=2) # 모든 경우에 수를 다 따져줌
