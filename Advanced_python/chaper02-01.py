...
# 그룹 리스트 선언
numbers = [str(n) for n in range(1,21)] # 리스트로 다음
ranks = 'A B C D'.split()


# List Comprehension
students = [Classses(rank, number) for rank in ranks for number in range(1, 21)]

print('EX5-1 -', len(students))
print('EX5-2 -', students[4].rank) # 딕셔너리 사용하듯 사용 가능


# 가독성
# # List Comprehension을 남발하고 길게 작성하면 가독성이 떨어짐

students2 = [Classses(rank, number) 
